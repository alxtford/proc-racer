...
const STYLE_CARD_MIN_WIDTH = 220;
const GRID_GAP_PX = 14;

function getStylePageSize(containerWidth = 0) {
  const fallbackWidth = typeof window !== "undefined" ? Math.max(window.innerWidth * 0.72, 640) : 640;
  const usableWidth = Math.max(containerWidth || fallbackWidth, STYLE_CARD_MIN_WIDTH);
  const stride = STYLE_CARD_MIN_WIDTH + GRID_GAP_PX;
  return Math.max(1, Math.min(4, Math.floor((usableWidth + GRID_GAP_PX) / stride)));
}
...
function getLaunchHint(state) {
  return state.save.settings.tutorialCompleted
    ? "Enter launch // D daily // Q remix // R reforge"
    : "Enter launch // D daily // Q skip // R reforge";
}
...
export function createUi(state, callbacks = {}) {
  ...
  function updateMenuScale() {
    ...
    scaleShell(refs.menuShell, "--menu-scale", 18);
  ...
  function getHomeBoardPage(displayedEvents, baseEvent) {
    ...
    const requestedPage = Number.isInteger(uiState.homeBoardPage) ? uiState.homeBoardPage : selectedPage;
    uiState.homeBoardPage = Math.max(0, Math.min(pageCount - 1, requestedPage));
    return {
      pageCount,
  ...
  function setHomePane(pane) {
    ...
    if (pane === "board" && !Number.isInteger(uiState.homeBoardPage)) {
      const displayedEvents = getDisplayedEvents(state);
      const selectedEvent = state.events[state.selectedEventIndex];
      const selectedIndex = Math.max(0, displayedEvents.findIndex((item) => item.id === selectedEvent?.id));
      uiState.homeBoardPage = Math.floor(selectedIndex / HOME_BOARD_PAGE_SIZE);
    }
    syncMenu();
  }
  ...
  function renderProfile() {
    ...
    const stylePageSize = getStylePageSize(refs.styleShop?.clientWidth || 0);
    const stylePageCount = Math.max(1, Math.ceil(slotItems.length / stylePageSize));
    uiState.stylePage = Math.min(uiState.stylePage || 0, stylePageCount - 1);
    const visibleSlotItems = slotItems.slice(
      uiState.stylePage * stylePageSize,
      (uiState.stylePage + 1) * stylePageSize,
    );
  ...
  function renderGarageRoll() {
    ...
    refs.garageRollGrid.innerHTML = roll.offers.map((offer) => {
      ...
      const compareTags = [
        ...
        `${getScrapValue(offer)} Scrap`,
      ];
      const targetCopy = hasCurrentCar
        ? `Target bay: ${currentCar.name} // ${formatCarMeta(currentCar)} // ${currentCar.role}`
        : "Target bay: Open slot // Keep this reveal to activate the bay instantly.";
          ...
          <div class="garage-roll-card-inner">
            ${isRevealed ? `
              ...
              <div class="roll-target-row">
                ...
                <div class="roll-target-copy">${targetCopy}</div>
              </div>
  ...
  function syncMenu() {
    ...
    if (refs.boardRerollBtn) {
      ...
      refs.boardRerollBtn.disabled = rerollDisabled;
